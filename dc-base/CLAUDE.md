...
**Basis und Abhaengigkeiten sind gepinnt (CC-ADR 0015).** `docker/Dockerfile` beginnt mit `FROM ruby:3.3.8@sha256:3e0e01e3…`; der Digest wird nur in einem eigenen, begruendeten Commit ausgetauscht. `bundle update` steht nicht mehr darin, und `Gemfile.lock` liegt im Build-Kontext — beides gehoert zusammen: ohne die Sperrdatei im Kontext loest `bundle install` frisch auf, und das Streichen von `bundle update` waere wirkungslos bei gleichzeitig erfuelltem Anschein.
...
| --- | --- | --- | --- |
| `260911` | `sha256:c41609c87d83e5222825af1878d791ac1345bcb00f0c127b517debe86b03ff93` | `ruby:3.3.8@sha256:3e0e01e30c20e1e3546c696ac8d7226fc93a34bbbd0b29eb6edb97ca49bf2ba8` | `0bebb5f` auf `feature/soyabud`, Dockerfile noch mit `bundle update`; aufgeloest: oydid 0.9.7, rails 7.2.3.2, json-ld 3.3.2, pagy 3.11.0, rbnacl 7.1.2, rdf 3.3.4, httparty 0.24.2 |
| `260911a` | `sha256:be6bc6261854b66468aa36381ae58ec51ad04c16afecb841eda5ef2a401c870b` | `ruby:3.3.8@sha256:3e0e01e30c20e1e3546c696ac8d7226fc93a34bbbd0b29eb6edb97ca49bf2ba8` | `a763ba1` auf `feature/soyabud`, erster gepinnter Bau: Basis per Digest, kein `bundle update`, Aufloesung aus `Gemfile.lock`. Geprueft: die aus dem Image gelesene `Gemfile.lock` ist mit der versionierten identisch, 39 Tests gruen |
| `260914` | `sha256:50961c549209b5eebd054fa1cf13541e97d2fea526a42d4858dd291ce341ab9e` | `ruby:3.3.8@sha256:3e0e01e30c20e1e3546c696ac8d7226fc93a34bbbd0b29eb6edb97ca49bf2ba8` | **Verworfen, nicht verwenden.** `748243c` auf `main`, aber ohne `--platform` auf einem Apple-Silicon-Rechner gebaut und deshalb `linux/arm64` — die Knoten sind `amd64`. Der Tag bleibt stehen, weil datierte Tags nach CC-ADR 0015 unveraenderlich sind; ueberschrieben wuerde er dieselbe Verschiebung wiederholen, gegen die die ADR geschrieben ist. Ersetzt durch `260914a` |
| `260914a` | `sha256:ae252839d495c4c849a8cf8467ccac78d116f4b69fe4650eebb041211624309a` | `ruby:3.3.8@sha256:3e0e01e30c20e1e3546c696ac8d7226fc93a34bbbd0b29eb6edb97ca49bf2ba8` | `748243c` auf `main`, erster Bau nach der Zusammenfuehrung (CC-ADR 0016): toter DID-Unterbau entfernt, Ruby 3.3.8, `RAILS_ROOT` repariert. `--platform linux/amd64`, Plattform am veroeffentlichten Index gegengelesen. 39 Tests gruen. `latest` zeigt auf denselben Digest |

**Immer mit `--platform linux/amd64` bauen**, oder ueber `build.sh`, das es setzt. Ein `docker build` ohne die Angabe erzeugt auf einem Apple-Silicon-Rechner ein `arm64`-Image, das auf den Knoten nicht laeuft — und `docker push` schiebt es widerspruchslos unter den amd64-Tag. Am 14.09.2026 genau so passiert, siehe Zeile `260914`. Die arm-Variante hat ihren eigenen Tag `arm64v8` und ihr eigenes Dockerfile; sie gehoert nie unter einen datierten Tag. Nach jedem Push die Plattform am Index gegenlesen: `docker buildx imagetools inspect oydeu/dc-base:<tag> | grep -i platform`.
...
**Die ganze Kette ist belegt — aber nur nativ, 13.09.2026.** Im nativ gebauten `oydeu/dc-base:arm64v8` braucht ein `Oydid.read` 0,72 bis 0,79 Sekunden, und `DidResolver.resolve` liefert fuer alle drei DIDs ein `DidDocument` in 0,7 bis 0,8 Sekunden — bei einem Budget von 5. Damit ist von der VDR bis zum `verify_key` alles in einem Stueck nachgewiesen, und das Budget hat gut sechsfachen Spielraum.

**Unter amd64-Emulation auf einem Mac ist die Resolver-Ebene dagegen nicht pruefbar.** Dort braucht dasselbe `Oydid.read` 7,5 bis 15 Sekunden — unter `oydid` 0.6.0 genauso wie unter 0.9.7 —, und `DidResolver::TIMEOUT` ist 5 Sekunden. Der Resolver laeuft also zwangslaeufig in `timeout resolving` und gibt `nil`. Das ist eine Aussage ueber die Emulation, nicht ueber die Bibliothek und nicht ueber den Resolver; das Budget bleibt, wo es ist. **Wer den Resolver messen will, baut vorher `--arm` und misst darin** — auf Apple-Silicon laeuft das nativ, der amd64-Bau nicht.
